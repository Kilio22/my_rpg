...
{
    for (int j = 0; j <= sprites->end_layout[2] - 1; j++) {
...
void check_sprite_position2(sprite_t *sprites, obj_t **obj, wind_t wind)
{
    for (int j = sprites->end_layout[2]; j <= sprites->end_layout[3] - 1; j++) {
        if (sprites->pos[j]->x < (obj[1]->pos.x - 416) ||
            sprites->pos[j]->x > (obj[1]->pos.x + 416) ||
            sprites->pos[j]->y < (obj[1]->pos.y - 256) ||
            sprites->pos[j]->y > (obj[1]->pos.y + 256))
...
{
    for (int i = 0; sprites[i] != NULL; i++)
        check_sprite_position2(sprites[i], obj, wind);
}